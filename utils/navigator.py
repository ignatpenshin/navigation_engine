...
class Navigator:
    ...
    def __init__(self):
        objects = Objects()
        rails = Rails(objects.get_LineStrings())
        freq = 1
        ...
        self.points_tree = self._init_kdtree(objects = objects)
    ...
    def _init_kdtree(self, objects: Objects) -> spatial.KDTree:
        kd_tree = spatial.KDTree([
                (row.geometry.x, row.geometry.y)
                for _, row 
                in self._snap_points(objects.get_Points()).iterrows()
        ])
        logging.info(f"Spatial Kd-Tree: inited")
        return kd_tree
    ...
    def _init_point_base(self, objects: Objects) -> dict:
        ...
        for idx, row in objects.get_Points().iterrows():
            point_base[counter] = [row.id, row.geometry, row.Tag, row.Description]
            counter += 1
        return point_base

    def _snap_points(self, points: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
        rail_id: int = self.main_rail["id"].values[0]
        points.geometry = points.apply(lambda row: \
                self.main_rail.interpolate(self.main_rail.project(row.geometry))[rail_id], \
                axis=1, result_type = 'expand')
        return points
    
    def _snap_pose(self, pose: Pose) -> Pose:
        rail_id: int = self.main_rail["id"].values[0]
        pose.coords = self.main_rail.interpolate(self.main_rail.project(pose.coords))[rail_id]
        return pose
    ...
    def tracking_info(self) -> None:
        ...
        while True:
            ...
            for i, dist in self.data_preapring():
                s = {"id": self.point_base[i][0],
                     "coords": (round(self.point_base[i][1].x, 6), \
                                round(self.point_base[i][1].y, 6)),
                     "Tag": self.point_base[i][2],
                     "Description": self.point_base[i][3],
                     "direct_m": dist}
                route_list.append(s)
